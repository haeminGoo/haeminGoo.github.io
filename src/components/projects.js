			...
			<article className="my-5" key={item.name}>
				...
				{item.img !== undefined &&
					<div className="flex justify-start">
						{item.img.map((data, i) => <a key={i} href={data.src} rel="noopener noreferrer" target="_blank" className='projectImg'>{data.src.includes('mp4') ? <video src={data.src} autoPlay muted alt={data.desc} width='200' /> : <img src={data.src} alt={data.desc} width='200' />}</a>)}
					</div>
				}
